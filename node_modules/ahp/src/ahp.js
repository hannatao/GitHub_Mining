...
class AHP {
    ...
    rankCriteriaItem(criterion, preferences) {
        ...
        for (let prefer of preferences) {
            ...
            if (Array.isArray(prefer)) {
                preferredItem = prefer[0]; 
                comparingItem = prefer[1];
                scale = prefer[2];
            } else {
    ...
    rankCriteria(preferences) {
        ...
        for (let prefer of preferences) {
            ...
            if (Array.isArray(prefer)) {
                preferredCriterion = prefer[0]; 
                comparingCriterion = prefer[1];
                scale = prefer[2];
            } else {
    ...
    findNextProblem() {
        ...
        var temp = AHP.findRankMatrixIncompleteCell(self.criteriaRank);
        let result = temp[0];
        let rowI = temp[1];
        let colI = temp[2];
        if (result) {
        ...
        for (let crit in self.criteriaItemRank) {
            var temp = AHP.findRankMatrixIncompleteCell(self.criteriaItemRank[crit]);
            let result = temp[0];
            let rowI = temp[1];
            let colI = temp[2];
            if (result) {
        ...
        let ci = AHP.calculateMatrixConsistency(self.criteriaRank).ci;
            let ri = AHP.calculateMatrixConsistency(self.criteriaRank).ri;
            let cr = AHP.calculateMatrixConsistency(self.criteriaRank).cr;
            let weightedVector = AHP.calculateMatrixConsistency(self.criteriaRank).weightedVector;
        if (cr > 0.1) {
        ...
        for (let j = 0; j < self.criteria.length; j++) {
            ...
            let ci = AHP.calculateMatrixConsistency(self.criteriaItemRank[crit]).ci;
            let ri = AHP.calculateMatrixConsistency(self.criteriaItemRank[crit]).ri;
            let cr = AHP.calculateMatrixConsistency(self.criteriaItemRank[crit]).cr;
            let weightedVector = AHP.calculateMatrixConsistency(self.criteriaItemRank[crit]).weightedVector;
            if (cr > 0.1) {
     ...
     */
    run() {
        let error = this.debug().error;
        let rankingMatrix = this.debug().rankingMatrix;
        let itemRankMetaMap = this.debug().itemRankMetaMap;
        let criteriaRankMetaMap = this.debug().criteriaRankMetaMap;
        let rankedScoreMap = this.debug().rankedScoreMap;
        return {
            error,
            rankingMatrix,
            itemRankMetaMap,
